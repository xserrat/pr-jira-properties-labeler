...
action::getJiraCodeFromPRTitle() {
    echo $(github::getPullRequestTitle) | sed -E "s/$(action::input::regexpJiraCodeOnPrTitle)/\1/"
}

action::addPriorityLabel() {
    local issue_code=$(action::getJiraCodeFromPRTitle)
    echo "Retrieving the priority of the issue code $issue_code from JIRA..."
    local issue_prio=$(jira::get_priority_of $issue_code)
...
action::run() {
    ...
    local action=$(github::getEventAction)
